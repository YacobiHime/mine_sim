...
│                                                             │
│  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────┐ │
│  │  mine_sim      │─►│  Gateプロキシ   │─►│  Forge鯖     │ │
│  │  エージェント   │  │  0.0.0.0:25565  │  │  :25566      │ │
│  └─────────────────┘  └─────────────────┘  └─────────────┘ │
│         ↑                           ↓           ↑          │
│      LLM判断                 Forge中継    MineColonies    │
└─────────────────────────────────────────────────────────────┘
                              ↕ 別のPC
                      ┌─────────────────┐
                      │  Ollama         │
...
**Gateプロキシについて:**
- Gate（minekube/gate）はForge 1.13-1.20.1のFML2/FML3ハンドシェイクをネイティブに中継
- mineflayerボット（バニラクライアント）がForgeサーバーに接続できるようにする
- `integrated_start.sh` 実行時に自動起動
...
**必要なもの:**
- Java 17 or 21
- Forge 47.1.3（Minecraft 1.20.1 用）
